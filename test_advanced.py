...
class advancedMemAgent:
    def __init__(self, model, backend, retrieve_k, temperature_c5):
        self.memory_system = AgenticMemorySystem(
            model_name="all-MiniLM-L6-v2", llm_backend=backend, llm_model=model
        )
...
def evaluate_dataset(
    ...
    memories_dir = os.path.join(
        ...
        "cached_memories_advanced_{}_{}".format(backend, model),
    )
    ...
    for sample_idx, sample in enumerate(samples):
        ...
        if os.path.exists(memory_cache_file):
            ...
            if os.path.exists(retriever_cache_file):
                ...
                print(
                    f"No retriever cache found at {retriever_cache_file}, loading from memory"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate text-only agent on LoComo dataset"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--temperature_c5", type=float, default=0.5, help="Temperature for the model"
    )
    parser.add_argument("--retrieve_k", type=int, default=10, help="Retrieve k")
